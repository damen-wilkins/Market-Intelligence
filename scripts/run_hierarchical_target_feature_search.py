...
TICKER = "SPY"
EXPERIMENT_NAME = "xlstm_flat_target_trend_search"
MODEL_NAME = "xlstm_flat_target_trend_search_v1"
...
def train_fold_stage(
    ...
    validation_sequences = preprocessor.build_inference_sequences(
        ...
        task=task,
        include_all=include_all_inference,
    )
...
def evaluate_candidate(
    ...
    for fold_number, (
        ...
        fold_batches.append(
            {
                "fold": fold_number,
                "target_dates": stage1["target_dates"],
    ...
    up_threshold_result = (
        ...
        .select_probability_threshold(
            ...
            positive_probabilities=stage2_probability[
                actual_move_mask
            ],
        )
    )
    ...
    stage2_predicted = (
        stage2_probability[actual_move_mask]
    ...
    for batch in fold_batches:
        ...
        fold_move_actual = stage1_actual[
            fold_slice
        ]
        fold_move_probability = stage1_probability[
            fold_slice
        ]
        fold_up_probability = stage2_probability[
            fold_slice
        ]

        fold_move_predicted = (
            fold_move_probability
            >= move_threshold_result["threshold"]
        ).astype(np.int64)

        fold_final_predicted = compose_hierarchical_predictions(
            move_probabilities=fold_move_probability,
            up_probabilities=fold_up_probability,
            move_threshold=float(
                move_threshold_result["threshold"]
            ),
            up_threshold=float(
        ...
        fold_move_metrics = binary_metrics(
            actual=fold_move_actual,
            predicted=fold_move_predicted,
            negative_name="FLAT",
            positive_name="MOVE",
        )
        ...
        fold_stage2_predicted = (
            fold_up_probability[fold_move_mask]
            >= up_threshold_result["threshold"]
        ).astype(np.int64)
        ...
        fold_metrics.append(
            {
                ...
                "stage1_balanced_accuracy": float(
                    fold_move_metrics["balanced_accuracy"]
                ),
                "stage1_flat_f1": float(
                    fold_move_metrics["per_class"]["FLAT"]["f1"]
                ),
                "stage1_move_f1": float(
                    fold_move_metrics["per_class"]["MOVE"]["f1"]
                ),
                ...
                "end_to_end_balanced_accuracy": float(
                    fold_end_to_end_metrics["balanced_accuracy"]
                ),
            }
        )
    ...
    fold_end_to_end_macro_f1 = np.asarray(
        [
            metrics["end_to_end_macro_f1"]
    ...
    return {
        ...
        "end_to_end_macro_f1_fold_mean": float(
            fold_end_to_end_macro_f1.mean()
        ),
        "end_to_end_macro_f1_fold_std": float(
            fold_end_to_end_macro_f1.std(ddof=0)
        ),
...
def build_candidate_datasets(
    ...
    for target in build_target_candidates():
        for feature_scope in FEATURE_SCOPES:
            candidate_name = (
                f"{target.name}__{feature_scope}"
            )
            ...
            datasets[candidate_name] = dataset
            metadata[candidate_name] = {
                "target_name": target.name,
                "target_description": target.description,
                "volatility_window": int(
                    target.volatility_window
                ),
                "threshold_multiplier": float(
                    target.threshold_multiplier
                ),
                "feature_scope": feature_scope,
...
def build_summary(
    ...
    for result in results:
        ...
        stage1 = result["metrics"]["stage1"]
        stage2 = result["metrics"][
            "stage2_oracle_move_rows"
        ]
        end_to_end = result["metrics"]["end_to_end"]

        rows.append(
            {
                "target_name": result["target_name"],
                "volatility_window": result[
                    "volatility_window"
                ],
                "threshold_multiplier": result[
                ...
                "train_down_share": distribution[
                    "down_share"
                ],
                "train_flat_share": distribution[
                    "flat_share"
                ],
                "train_up_share": distribution[
                    "up_share"
                ],
                "stage1_balanced_accuracy": stage1[
                    "balanced_accuracy"
                ],
                "stage1_macro_f1": stage1[
                    "macro_f1"
                ],
                ...
                "stage1_move_f1": stage1[
                    "per_class"
                ]["MOVE"]["f1"],
                "stage2_balanced_accuracy": stage2[
                    "balanced_accuracy"
                ],
                "stage2_macro_f1": stage2[
                    "macro_f1"
                ],
                ...
                "end_to_end_macro_f1": end_to_end[
                    "macro_f1"
                ],
                ...
                "move_threshold": result["metrics"][
                    "move_threshold"
                ],
                "up_threshold": result["metrics"][
                    "up_threshold"
                ],
                "end_to_end_macro_f1_fold_std": result[
                    "metrics"
    ...
    for metric in [
        ...
        summary[
            ...
            lambda row: (
                ...
                if row["feature_scope"] == "base_trend"
                else 0.0
            ),
            axis=1,
        )
...
def print_summary(
    ...
    print(
        "TRAIN-ONLY FLAT TARGET x TREND FEATURE SEARCH"
    )
    print(
        "============================================================"
    )
    print(
        "These rows use the SAME common dates and LOCKED hierarchical "
        "xLSTM parameters."
    )
    print(
    ...
    display = summary[
        [
            ...
            "end_to_end_macro_f1",
            "end_to_end_macro_f1_fold_std",
            "trend_delta_stage1_balanced_accuracy",
            "trend_delta_stage1_flat_f1",
        ]
    ].copy()
    ...
    print()
    print(
        "Interpretation rule: do NOT choose a FLAT definition only because "
        "its 3-class Macro F1 is highest; changing the threshold changes the "
        "meaning of the target. First look for materially better FLAT "
        "discrimination, stable folds, and preserved UP/DOWN discrimination."
    )
...
def main():
    ...
    print(
        "Building target x feature candidate datasets..."
    )

    datasets, candidate_metadata = (
    ...
    print(
        "This sweep does NOT run Optuna. It isolates target-definition "
        "and trend-feature effects before any winner is retuned."
    )

    split_reference_name = next(
        iter(datasets)
    )
    ...
    for candidate_number, candidate_name in enumerate(
        ...
        dataset = datasets[candidate_name]
        train = (
            dataset[
                pd.to_datetime(
                    dataset["target_date"]
        ...
        if set(
            ...
            raise ValueError(
                f"Training dates are not aligned for {candidate_name}."
            )

        print()
        print(
            f"[{candidate_number}/{len(candidate_names)}] "
            f"{metadata['target_name']} | {metadata['feature_scope']}"
        )
        print(
            f"  target: {metadata['volatility_window']}d x "
            f"{metadata['threshold_multiplier']:.2f}"
        )
        print(
    ...
    summary_path = (
        EXPERIMENT_DIRECTORY
        / f"flat_target_trend_model_search_{timestamp}.csv"
    )
    summary.to_csv(
        summary_path,
        index=False,
    ...
    experiment_path = ExperimentTracker(
        ...
        parameters={
            "reference_hierarchical_experiment": str(
                reference_path
            ),
            "cv_splits": CV_SPLITS,
            ...
            "target_candidates": [
                {
                    "name": candidate.name,
                    "volatility_window": candidate.volatility_window,
                    "threshold_multiplier": candidate.threshold_multiplier,
                    "description": candidate.description,
                }
                for candidate in build_target_candidates()
            ],
            "feature_scopes": list(
                FEATURE_SCOPES
            ),
            "selection_policy": (
                "Research sweep only. No target or feature scope is promoted "
                "automatically. Candidates use common dates and locked model "
                "parameters. Any selected winner must be independently retuned "
                "using train-only CV before outer-validation evaluation."
            ),
        },
        metrics={
            "candidates": results,
