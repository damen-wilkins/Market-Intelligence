...
from app.training.torch_classifier_serializer import TorchClassifierSerializer
...
MODEL_NAME = "xlstm_breadth_subset_selection"
MODEL_VERSION = "xlstm_breadth_subset_v1"

EXPERIMENT_DIRECTORY = Path("experiments")

MODEL_PATH = Path(
    "models/xlstm_direction_classifier_breadth_selected.pt"
)

VALIDATION_OUTPUT_PATH = Path(
    "models/xlstm_direction_breadth_selected_validation_predictions.csv"
)
...
BREADTH_ALIASES = {
    "rsp_relative_return": "rsp",
    "sector_positive_participation": "participation",
    "sector_return_dispersion": "dispersion",
    "sector_average_correlation_20d": "correlation",
    "sector_volume_breadth": "volume",
}
...
def load_reference_experiment() -> tuple[Path, dict]:
    matches = []
    ...
    if not matches:
        raise FileNotFoundError(
            "No original xLSTM reference experiment was found."
        )
...
def validate_reference_experiment(
    ...
    if missing_parameters:
        raise ValueError(
            "Reference xLSTM experiment is missing parameters: "
    ...
    if reference_features != expected_base_features:
        raise ValueError(
            "Reference xLSTM feature contract does not match "
    ...
    missing_aliases = [
        ...
        if feature not in BREADTH_ALIASES
    ]

    if missing_aliases:
        raise ValueError(
            "Breadth aliases are missing for features: "
            f"{missing_aliases}"
        )


def build_candidate_name(
    breadth_features: tuple[str, ...],
) -> str:
    if not breadth_features:
        return "baseline"

    return "+".join(
        BREADTH_ALIASES[feature]
        for feature in breadth_features
    )


def build_breadth_subsets() -> list[tuple[str, ...]]:
    breadth_features = tuple(
        DirectionFeatureBuilder
        .BREADTH_FEATURE_COLUMNS
    )
...
def evaluate_feature_subset_cv(
    ...
    return {
        ...
        "breadth_features": list(
            breadth_features
        ),
        "feature_count": len(
            feature_columns
        ),
        "cv_macro_f1": mean_metric(
            "macro_f1"
        ),
        ...
        "cv_balanced_accuracy": mean_metric(
            "balanced_accuracy"
        ),
        "cv_accuracy": mean_metric(
            "accuracy"
        ),
        "cv_down_f1": mean_metric(
            "down_f1"
        ),
        "cv_flat_f1": mean_metric(
            "flat_f1"
        ),
        "cv_up_f1": mean_metric(
            "up_f1"
        ),
...
def build_validation_output(
    ...
    if not baseline_dates.equals(
        ...
        raise ValueError(
            "Baseline and selected validation target dates do not align."
        )
    ...
    if not np.array_equal(
        ...
        raise ValueError(
            "Baseline and selected validation labels do not align."
        )
        ...
        rows.append(
            {
                ...
                "breadth_feature_count": len(
                    result[
                        "breadth_features"
                    ]
                ),
                "breadth_features": "|".join(
                    result[
                        "breadth_features"
                    ]
                ),
...
def print_cv_summary(
    ...
    print(
        "xLSTM BREADTH SUBSET CV RESULTS"
    )
    print(
        "=============================================="
    )
    ...
    return (
        ...
        .save(
            ...
            parameters={
                ...
                "cv_splits": CV_SPLITS,
                "selection_gate": (
                    "Positive mean Macro F1 delta versus controlled "
                    "baseline and improvement in at least 2 of 3 folds."
                ),
            },
            metrics={
                "controlled_baseline": (
...
def main():
    ...
    print(
        "Reference xLSTM parameters are locked."
    )

    print(
        "No Optuna retuning will occur during feature selection."
    )
    ...
    print(
        f"Breadth subsets to evaluate: {len(breadth_subsets)}"
    )
    ...
    for (
        ...
        print()
        print(
            f"[{candidate_number}/{len(breadth_subsets)}] "
    ...
    if selected_candidate is None:
        ...
        print()
        print(
            "No breadth subset passed the train-only selection gate."
        )

        print(
            "Outer validation was NOT re-evaluated."
        )
    ...
    selected_breadth_features = list(
        selected_candidate[
            "breadth_features"
        ]
    )
    ...
    selected_feature_columns = [
        *DirectionFeatureBuilder
        .BASE_FEATURE_COLUMNS,
        *selected_breadth_features,
    ]

    print()
    print(
        "Selected breadth subset:"
    )
    ...
    print()
    print(
        "Training controlled baseline on full training data..."
    )
    ...
    print(
        "Training selected breadth candidate on full training data..."
    )
    ...
    if (
        "macro_f1"
        in reference_validation_metrics
    ):
        ...
        print(
            "Controlled baseline reproduction delta:",
            round(
                baseline_metrics[
                    "macro_f1"
                ]
    ...
    metadata = {
        ...
        "horizon": HORIZON,
        "architecture": (
            "Official NX-AI xLSTMBlockStack "
            "using mLSTM blocks"
        ),
        "reference_experiment": (
            str(reference_path)
        ...
        "feature_selection": {
            "method": (
                "Exhaustive evaluation of all 32 subsets of the "
                "five breadth proxy features using fixed reference "
                "xLSTM parameters and train-only 3-fold walk-forward CV."
            ),
            "selection_gate": (
                "Positive mean Macro F1 delta versus controlled "
                "baseline and improvement in at least 2 of 3 folds."
        ...
        "experiment_notes": (
            "Breadth feature selection was performed only on the "
            "training period. Hyperparameters were not retuned during "
            "feature selection. The outer validation period was used "
            "only after selecting the breadth subset. Held-out test "
